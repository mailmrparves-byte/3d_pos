...
router.get('/', authenticateToken, async (req, res) => {
  try {
    ...
    const [
      ...
      pool.query(`SELECT COALESCE(SUM(grand_total),0) as total, COUNT(*) as count FROM sales WHERE DATE(created_at) = $1 AND is_draft = false`, [today]),
      ...
      pool.query(`SELECT COUNT(*) FROM products WHERE stock_qty <= low_stock_threshold`),
      pool.query(`SELECT COUNT(*) FROM customers WHERE outstanding_balance > 0`),
      ...
      pool.query(`
        ...
        FROM sales s WHERE s.is_draft = false ORDER BY s.created_at DESC LIMIT 10`),
      pool.query(`
        ...
        FROM products p WHERE p.stock_qty <= p.low_stock_threshold ORDER BY p.stock_qty ASC LIMIT 20`)
    ]);
